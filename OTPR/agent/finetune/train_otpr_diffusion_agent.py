"""
OT Implicit diffusion Q-learning (OTPR) trainer for diffusion policy.
...
class TrainOTPRDiffusionAgent(TrainAgent):
    ...
    def __init__(self, cfg):
        ...
        # Sampling
        self.cfg_num_sample = cfg.train.eval_sample_num
        
        # OT_parameters
        
        self.OT_epsilon = cfg.train.OT_epsilon
        self.OT_num_hidden_layers = cfg.train.OT_num_hidden_layers
        self.OT_dim_hidden_layers = cfg.train.OT_dim_hidden_layers
        self.OT_act_function = cfg.train.OT_act_function
        if cfg.model.otsolver.ot_type == "unsupervised":
            self.OT_dataset_train = None
        else:
            self.OT_dataset_train = hydra.utils.instantiate(cfg.train_dataset)
    ...
    def run(self):
        ...
        while self.itr < self.n_train_itr:
            ...
            self.num_sample = 1  if eval_mode else self.num_sample = self.cfg_num_sample
            ...
            if not eval_mode:
                ...
                if self.itr >= self.n_critic_warmup_itr:
                    obs_OT = torch.from_numpy(obs_trajs).float().to(self.device)
                    action_OT = torch.from_numpy(action_trajs).float().to(self.device)
                    self.model.train_OT(obs_OT, action_OT,
                                        self.OT_epsilon, self.OT_num_hidden_layers, self.OT_dim_hidden_layers, self.OT_act_function, self.OT_dataset_train)
