...
import torch
from model.common.mlp import Potential
import itertools
import os
import numpy as np
...
class LargeScaleOTSolver(object):
    ...
    def feed_unsupervised_OT_params(self,cost="l2",epsilon=1e-6,u_input_size=64*64, v_input_size=64*64, num_hidden_layers=3,
        ...
        self.potential_u_dict = {"input_size":u_input_size,"num_hidden_layers":num_hidden_layers,
                               "dim_hidden_layers":dim_hidden_layers,"act_fun":act_function}
        self.potential_v_dict = {"input_size":v_input_size,"num_hidden_layers":num_hidden_layers,
                               "dim_hidden_layers":dim_hidden_layers,"act_fun":act_function}

        self.param_fed = True

        ###### building networks
        self.u_net = Potential(self.potential_u_dict).cuda()
        self.v_net = Potential(self.potential_v_dict).cuda()

    def feed_semi_supervised_OT_params(self, cost="l2", epsilon=1e-6, alpha = 1, tau = 0.1, tau_= None,
                                       u_input_size=64*64, v_input_size=64*64, num_hidden_layers=3,
                                       dim_hidden_layers=512, act_function="silu"):
        '''
        ...
        self.tau = tau
        self.alpha = alpha
        self.u_potential_dict = {"input_size": u_input_size, "num_hidden_layers": num_hidden_layers,
                               "dim_hidden_layers": dim_hidden_layers, "act_fun": act_function}
        self.v_potential_dict = {"input_size": v_input_size, "num_hidden_layers": num_hidden_layers,
                               "dim_hidden_layers": dim_hidden_layers, "act_fun": act_function}
        ...
        ###### building networks
        self.u_net = Potential(self.u_potential_dict).cuda()
        self.v_net = Potential(self.v_potential_dict).cuda()
    ...
    def computeGuidingMatrix(self,Xs_batch,Xt_batch,xs_paired,xt_paired):
        ...
        '''
        with torch.no_grad():
            Cs = self.cost_matrix_cosine(Xs_batch, xs_paired)
            Ct = self.cost_matrix_cosine(Xt_batch, xt_paired)
    ...
    def cost_matrix_cosine(self, x, y):
    ...
    def cost_matrix(self,x, y):
        ...
        if self.cost == "l2":
            ...
            if callable(self.cost):
                ...
                x1, y1 = x.size(0), y.size(0)  
                x = torch.reshape(x, (x1, -1))
                y = torch.reshape(y, (y1, -1))
                d_x = x.size(1)
                d_y = y.size(1)
                x_expanded = x.unsqueeze(1).expand(x1, y1, d_x)  # Shape: (x_len, y_len, d_x)  
                y_expanded = y.unsqueeze(0).expand(x1, y1, d_y) # Shape: (x_len, y_len, d_y)  
                x = torch.reshape(x_expanded, (x1 * y1, d_x))
                y = torch.reshape(y_expanded, (x1 * y1, d_y))
                cost = torch.reshape(self.cost(x, y), (x1, y1))
    ...
    def train(self,source_dataset,target_dataset, paired_dataset = None, batch_size=256,num_workers=0,
        ...
        source_dataset = TensorDataset(source_dataset)  
        target_dataset = TensorDataset(target_dataset)  
        ...
        if paired_dataset is None:
            ...
            self.paired_loader = DataLoader(paired_dataset, batch_size=batch_size, num_workers=num_workers,
                                       drop_last=True)
        ...
        for step in tqdm_range:
            ...
            if paired_dataset is not None:
                try:
                    xt_k,xs_k = next(paired_iter)
                    xs_k = xs_k['state']
                except:
                    paired_iter = iter(self.paired_loader)
                    xt_k,xs_k = next(paired_iter)
                    xs_k = xs_k['state']
                    xs_k,xt_k = xs_k.cuda(),xt_k.cuda()
                    # print(xs_k.shape,xt_k.shape, "_k")
            ...
            if self.ot_type == "semi-supervised":
                ...
                xs_k,xt_k = xs_k.view(batch_size,-1),xt_k.view(batch_size,-1)
    ...
    def get_h(self, source_dataset, target_dataset):
        batch_size = source_dataset.size(0)
        # if self.ot_type == "semi-supervised":
        ...
        #         source_keypoints.append(paired_dataset[i][1]['state'].unsqueeze(0))
        #         target_keypoints.append(paired_dataset[i][0].unsqueeze(0))
        #     source_keypoints = torch.cat(source_keypoints,dim=0).cuda()
        #     target_keypoints = torch.cat(target_keypoints,dim=0).cuda()
            
        
        with torch.no_grad(): 
            u_batch = self.u_net(source_dataset)
            v_batch = self.v_net(torch.reshape(target_dataset, (batch_size, -1)))
    
        # u_batch = self.compute_potentials(source_dataset, self.u_net)
        # v_batch = self.compute_potentials(target_dataset, self.v_net)
        # if self.ot_type == "semi-supervised":
        #     source_dataset,target_dataset, source_keypoints,target_keypoints = torch.reshape(source_dataset, (batch_size, -1)),torch.reshape(target_dataset, (batch_size, -1)), torch.reshape(source_keypoints, (batch_size, -1)),torch.reshape(target_keypoints, (batch_size, -1))
        #     G = self.computeGuidingMatrix(source_dataset,target_dataset, source_keypoints,target_keypoints)
        #     C = self.cost_matrix(source_dataset,target_dataset)
        ...
        G = self.cost_vector(source_dataset,target_dataset)
    
        V = torch.reshape(u_batch, (-1, 1)) + torch.reshape(v_batch, (-1, 1)) - torch.reshape(G, (-1, 1))
        h = (1. / (2. * self.epsilon)) * torch.max(torch.zeros(1).to(V.device), V) + 0.0000001
