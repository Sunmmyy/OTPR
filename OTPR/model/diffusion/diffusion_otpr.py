"""
OT Implicit diffusion Q-learning (OTPR) for diffusion policy.
...
class OTPRDiffusion(RWRDiffusion):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.otsolver = otsolver #OT_solver
        self.OT_trained = False
    ...
    #OT_cost
    def get_cost(self, obs, actions):
        with torch.no_grad():
            current_q1, current_q2 = self.target_q({"state": obs}, actions)
            # return -current_q1
            return -1 * torch.min(current_q1, current_q2)
            # return -1 * (current_q1+current_q2)/2
     
    
    #OT_solver Train
    def train_OT(self, source_dataset, target_dataset,  epsilon, num_hidden_layers, dim_hidden_layers, act_function, paired_dataset=None):
        self.OT_batch_size = 128
        self.OT_lr = 1e-5
        self.OT_iterations = 1000
        self.OT_save_dir = None
        self.OT_trained = True

        
        
        if self.otsolver.ot_type == "unsupervised":
            self.otsolver.feed_unsupervised_OT_params(cost=self.get_cost, epsilon=epsilon, 
                                                    u_input_size=self.obs_dim,
                                                    v_input_size=self.action_dim*self.horizon_steps,
                                                    num_hidden_layers=num_hidden_layers,
                                                    dim_hidden_layers=dim_hidden_layers,
                                                    act_function=act_function )
            self.otsolver.train(source_dataset, target_dataset, batch_size=self.OT_batch_size, 
                                lr=self.OT_lr, num_train_steps=self.OT_iterations, save_dir=self.OT_save_dir)
        else:
            self.otsolver.feed_semi_supervised_OT_params(cost=self.get_cost, epsilon=epsilon, 
                                                    u_input_size=self.obs_dim,
                                                    v_input_size=self.action_dim*self.horizon_steps,
                                                    num_hidden_layers=num_hidden_layers,
                                                    dim_hidden_layers=dim_hidden_layers,
                                                    act_function=act_function )
            self.otsolver.train(source_dataset, target_dataset, paired_dataset, batch_size=self.OT_batch_size, 
                                lr=self.OT_lr, num_train_steps=self.OT_iterations, save_dir=self.OT_save_dir)
    ...
    #     # Predict
    #     x_recon = self.network(x_noisy, t, cond=cond)
    #     h_weight = self.otsolver.get_h(cond, x_recon).squeeze().detach()
    ...
    def forward(
        ...
        samples = super(OTPRDiffusion, self).forward(
            {"state": cond_repeat},
        ...
        #get h
        if self.OT_trained:
            h = self.otsolver.get_h(cond_repeat, samples).detach()
            h = h.view(S, B)
        ...
        if deterministic or (not use_expectile_exploration):
            ...
            random_indices = torch.randint(0, S, (B,)).cuda() 
            sample_indices = random_indices[None, :, None, None]
